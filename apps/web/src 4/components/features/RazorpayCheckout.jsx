...
const RazorpayCheckout = ({ product = {}, className = '', buttonText = 'Buy Now' }) => {
  ...
  const handleApplyCoupon = () => {
    // Just a conceptual coupon feature for UI interaction logic. 
    // Usually validated against backend.
    if (couponCode.toUpperCase() === 'DISCOUNT50') {
      setDiscountPercent(50);
      toast({ title: 'Coupon Applied', description: '50% discount has been applied!' });
  ...
  const finalPrice = discountPercent > 0 ? Math.max(0, Math.round(basePrice * (1 - discountPercent / 100))) : basePrice;
    ...
    >
      <form onSubmit={handleCheckout} className="w-full space-y-5">
        ...
        <motion.div
          ...
          initial={{ opacity: 0.3, pointerEvents: 'none' }}
          animate={{ opacity: (isEmailVerified && isValidPhone) ? 1 : 0.3, pointerEvents: (isEmailVerified && isValidPhone) ? 'auto' : 'none' }}
          transition={{ duration: 0.4 }}
        >
          <label className="text-[10px] font-semibold text-white/35 uppercase tracking-[0.15em] ml-1">
          ...
          <div className="flex gap-3">
            <input
              ...
              disabled={!(isEmailVerified && isValidPhone)}
              placeholder="Enter coupon code"
              className="w-full px-4 py-4 rounded-xl bg-white/[0.03] border border-white/[0.06] text-white placeholder-white/12 focus:outline-none focus:border-[#E2F034]/30 transition-all duration-300 text-sm disabled:opacity-40 uppercase"
            />
            <button
              type="button"
              className="px-6 py-4 rounded-xl bg-white/[0.05] border border-white/[0.1] text-white/80 font-bold uppercase hover:bg-white/[0.1] hover:text-white transition-all text-sm disabled:opacity-40 whitespace-nowrap"
              disabled={!(isEmailVerified && isValidPhone) || !couponCode}
          ...
          {discountPercent > 0 && (
            <p className="text-green-400 text-xs mt-2 ml-1 flex items-center gap-1.5 font-medium"><CheckCircle2 size={12} /> {discountPercent}% Off Coupon Applied Successfully</p>
          )}
        </motion.div>
