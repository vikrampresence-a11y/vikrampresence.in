...
import { ArrowRight, Book, MonitorPlay, Layers, ShieldCheck, Zap, Mail, Youtube, Instagram, ChevronDown, Plus, CheckCircle, X } from 'lucide-react';
...
// All marquee images — we need at least 3 unique ones to display
...
const HomePage = () => {
  ...
  // Live settings from admin
  const heroTitle = settings.heroTitle || 'Your Favourite';
  const heroSubtitle = settings.heroSubtitle || 'Motivational Speaker';
  const heroCtaText = settings.heroCtaText || 'Explore Now';
  const heroCtaLink = settings.heroCtaLink || '/shop';
  const heroSubtext = settings.heroSubtext || 'the ultimate collection of ebooks & courses to build clarity, discipline, and confidence.';
  const seoTitle = settings.seoTitle || 'Vikram Presence — Premium Ebooks & Courses';
  const seoDesc = settings.seoDescription || 'Premium ebooks and courses for building clarity, discipline, and absolute confidence.';
  // Use marquee images from admin if available, else default
  const adminMarqueeImages = settings.marqueeImages && settings.marqueeImages.length >= 3 ? settings.marqueeImages : null;
  const LIVE_MARQUEE = adminMarqueeImages || [
    '/assets/marquee/marquee-1.png',
    '/assets/marquee/marquee-2.png',
    '/assets/marquee/marquee-3.png',
    '/assets/marquee/marquee-4.png',
  ...
  const showMarqueeLive = LIVE_MARQUEE.length >= 3;
  const marqueeImagesDup = showMarqueeLive ? [...LIVE_MARQUEE, ...LIVE_MARQUEE] : [];

  // Announcement banner from admin
  const showAnnouncement = settings.announcementEnabled && settings.announcementText && !announcementDismissed;
  ...
  // Require at least 3 unique marquee images to display the slider
  const showMarquee = MARQUEE_IMAGES.length >= 3;
  // Duplicate images for seamless infinite scroll
  const marqueeImages = showMarquee ? [...MARQUEE_IMAGES, ...MARQUEE_IMAGES] : [];
  ...
  return (
    <>
      ...
      {/* Announcement Banner — controlled from admin */}
      <AnimatePresence>
        {showAnnouncement && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
            className="flex items-center justify-center gap-3 px-4 py-2.5 text-center text-xs font-bold text-black"
            style={{ background: settings.announcementColor || '#E2F034' }}>
            <span>{settings.announcementText}</span>
            <button onClick={() => setAnnouncementDismissed(true)} className="ml-2 opacity-60 hover:opacity-100"><X size={13} /></button>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="min-h-screen text-white font-sans relative overflow-hidden" style={{ background: 'var(--surface-0)' }}>
        ...
        <section className="relative min-h-screen flex items-center justify-center overflow-hidden grain-overlay radial-gold-glow section-glow">
          ...
          <div className="relative z-10 container mx-auto px-6 text-center flex flex-col items-center justify-center pt-28 pb-20">
            ...
            >
              ...
              <br />
              <span className="text-[#E2F034]" style={{ textShadow: '0 0 60px rgba(226, 240, 52, 0.15)' }}>{heroSubtitle}</span>
            </motion.h1>

            {/* Scrolling Product Marquee — from admin settings */}
            {showMarqueeLive && (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.8, delay: 0.35 }}
                className="w-full max-w-3xl mx-auto mb-8 overflow-hidden relative"
                style={{ maskImage: 'linear-gradient(to right, transparent, black 10%, black 90%, transparent)', WebkitMaskImage: 'linear-gradient(to right, transparent, black 10%, black 90%, transparent)' }}
              >
                <div className="flex gap-4 animate-marquee" style={{ width: 'max-content' }}>
                  {marqueeImagesDup.map((src, i) => (
                    <div key={i} className="w-64 h-36 rounded-2xl overflow-hidden border border-white/[0.08] shrink-0 group">
                      <img src={src} alt={`Slide ${(i % LIVE_MARQUEE.length) + 1}`} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                    </div>
                  ))}
                </div>
              </motion.div>
        ...
        {/* ═══════════════════════════════════════
            5. SOCIAL PROOF PILLS — moved to bottom
            ═══════════════════════════════════════ */}
        <section className="py-16 relative section-glow-blue">
          <div className="container mx-auto px-6 max-w-xl">
            <ScrollReveal>
              <div className="text-center mb-8">
                <p className="serif-italic text-white/35 text-lg">Join our community of</p>
                <h2 className="text-2xl font-bold tracking-tight text-white mt-1">
                  <span className="text-[#E2F034]">275K+</span> Followers
                </h2>
              </div>
